...
MCP tool call (server.py)  ──asyncio.to_thread──▶  mcp_v2.*
  ...
  │            └─ retrieval=bm25 (mode-first, config) or lancedb import absent (Intel Mac; safety net)
  │               → search_lexical (BM25 over Symbol FTS index; heuristic scan fallback)
...
| Tool | Backing | Notes |
| --- | --- | --- |
| `search` | Lance vector/hybrid with 3-list RRF (vector + graph + BM25), or BM25 lexical (`retrieval=bm25` or import-absent fallback) | dedup by FQN; role weights via `search_scoring`; list-set + `k` via injectable `RankConfig` |
