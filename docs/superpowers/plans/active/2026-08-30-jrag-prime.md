...
- Produces (all later tasks rely on these exact names):
  - `PRIME_TEMPLATE: str` — module constant; the canonical template from the spec (identity paragraph ending "You are the explorer; jrag is the map.", trust-rule line, `**Index state**` bullets, `**Commands by group** (from \`jrag --help\`)` block with `http-routes`/`http-clients` normalized, `**Command reference**` with all 33 one-liners verbatim from `jrag --help`, closing "Run `jrag <command> --help` for flags." line) with `{slot}` placeholders for computed values.
