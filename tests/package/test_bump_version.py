...
def test_apply_preserves_other_lines(tmp_path: Path) -> None:
    """Targeted replace: the ``name =`` lines survive the write intact."""
    _write_repo(tmp_path)
    result = _run("apply", "0.13.0", tmp_path)
    assert result.returncode == 0, result.stderr

    assert 'name = "jrag-cli"' in (tmp_path / "pyproject.toml").read_text(encoding="utf-8")
    assert 'name = "java-codebase-rag"' in (
        tmp_path / "shim" / "pyproject.toml"
